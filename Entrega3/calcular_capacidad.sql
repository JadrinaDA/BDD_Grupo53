...
FOR tupla_muelle IN SELECT permisos.pid, insts.iid, insts.capacidad, permisos.fecha_atraque FROM ((SELECT atraques.pid, instas.iid, instas.capacidad FROM
 (SELECT int_p.iid, tipo, capacidad FROM (SELECT iid FROM pertenece_a WHERE nombre_puerto = puerto) AS int_p INNER JOIN instalaciones ON int_p.iid = instalaciones.iid WHERE tipo = 'muelle') AS instas
...
END LOOP;
atracados_s := 0;
FOR tupla_inst IN SELECT int_p.iid, tipo, capacidad FROM (SELECT iid FROM pertenece_a WHERE nombre_puerto = nombre_puerto) AS int_p INNER JOIN instalaciones ON int_p.iid = instalaciones.iid WHERE tipo = 'muelle'
LOOP
...
FOR tupla_asti_2 IN SELECT permisos_astillero.pid, astis.iid, astis.capacidad, astis.fecha_atraque, permisos_astillero.fecha_salida FROM ((SELECT permisos.pid, insts.iid, insts.capacidad, permisos.fecha_atraque FROM ((SELECT pid, instas.iid, instas.capacidad FROM 
	((SELECT int_p.iid, tipo, capacidad FROM (SELECT iid FROM pertenece_a WHERE nombre_puerto = puerto) AS int_p INNER JOIN instalaciones ON int_p.iid = instalaciones.iid WHERE tipo = 'astillero')) AS instas
