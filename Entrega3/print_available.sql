...
END LOOP;
RETURN QUERY EXECUTE 'SELECT * FROM tabla_aux INNER JOIN (SELECT * FROM instalaciones WHERE instalaciones.tipo = tipo_chosen) AS middle  ON instalaciones.iid = middle.iid';
